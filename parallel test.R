...
# simuloop <- function(myseed){
#   
#   T0 = 20; T_obs = T0+10;
#   N_ctrl = 15; N = N_ctrl+1;
#   N_x = 2
#   ##Data Generating
#   datatoana <- DGP1(T_obs = 30, T0 = 20, N_ctrl =15)
#   
...
#   dat <- as.matrix(cbind(df[,1], rep(1,T_obs), df[,-1]))
# 
#   ##main algorithm
#   ## MIO part
#   err_messager <- 0
...
# making cluster and export package and source####
cl <- makeCluster(2)

#clusterExport(cl, "simuloop")             
clusterEvalQ(cl, 
             c(
               library(dplyr),library(ggplot2),library(caret),library(glmnet),
               library(knitr),library(gurobi),library(Matrix),library(leaps),
               library(gsynth),library(Synth),library(xtable),library(MCPanel)
             )
) 
#dgp
clusterCall(cl, function() { source("C:/Users/user/Documents/Thesis/Oct_CodingSummary/Simulation/DGP.R",
                                    encoding = "utf-8")})
#method
clusterCall(cl, function() { source("C:/Users/user/Documents/Thesis/Oct_CodingSummary/Simulation/Methods.R",
                                    encoding = "utf-8")})
#mygsynth and aoa
clusterCall(cl, function() { source("C:/Users/user/Documents/Thesis/Oct_CodingSummary/Simulation/mygsynth.R",
                                    encoding = "utf-8")})
#soa
clusterCall(cl, function() { source("C:/Users/user/Documents/Thesis/Oct_CodingSummary/Simulation/Simultaneous Optimization Approach.R",
                                    encoding = "utf-8")})
#MC
clusterCall(cl, function() { source("C:/Users/user/Documents/Thesis/Oct_CodingSummary/Simulation/MCoptim.R",
                                    encoding = "utf-8")})
...
output <- foreach(myseed = 1:B,
  ...
  N_x = 2
  ##Data Generating
  datatoana <- DGP4(T_obs = T_obs, T0 = T0, N_ctrl = N_ctrl)
...
#averaging####
msetable0 = matrix(0,21,3)
count = 0
for (i in 1:B){
  if (is.null(output[[i]])){
    count = count
  } else{
    count = count + all(!(output[[i]] == 0 ))
    msetable0 = msetable0 + output[[i]]
  }
}


msetable <- msetable0/count

#write.csv(msetable, file = "C:/Users/user/Documents/Thesis/Oct_CodingSummary/Simulation/simuresult/msetable_dgp3t40n10.csv")
